...
1. Create `frontend/src/state.rs` with `ChatAction` enum and reducer
2. Create `ChatStateProvider` component using `use_reducer`
3. Add `state.rs` to `frontend/src/lib.rs` module tree
4. Wrap root component with provider in `main.rs`
5. Refactor `use_chat_websocket` hook — replace `state_ref` with context
6. Refactor `use_channels` hook — replace `state_ref` with context
7. Refactor `main.rs` callbacks — extract to `logic.rs`, use dispatch
8. Remove `state_ref` and manual sync from `main.rs`
9. Run `cargo check --target wasm32-unknown-unknown` and fix any issues
10. Run existing frontend tests — ensure no regressions
